...
## 1 Le prime basi
 ...
 ### Operatori e metodi
   ...
   Gli operatori di assegnazione pag 55  
         ...
         Gli operatori di assegnazione, come (=) e gli operatori composti come +=, -=, *=,  
         /=  e  %=,  servono  per  assegnare  valori  a  variabili.  

   Operatori avanzati
      Oltre  a  questi,  Rust  supporta  anche  operatori  più  avanzati  come  quelli  per  la 
      manipolazione dei bit (&, |, ^, <<, >>) e il pattern matching. 

      tabella riassuntiva
               +----------+----------------------------------------------+
               | Operatore| Utilizzo                                     |
               +----------+----------------------------------------------+
               | =        | Attribuisce un valore (immutabile)           |
               |          | a una variabile                              |
               |          | Esempio: let x = 3;                          |
               +----------+----------------------------------------------+
               | ==       | Confronta l'uguaglianza tra valori           |
               |          | Esempio: if x == 3 { ... }                   |
               +----------+----------------------------------------------+
               | !=       | Confronta la differenza tra valori           |
               |          | Esempio: if x != 3 { ... }                   |
               +----------+----------------------------------------------+
               | < , >    | Confronta se un valore è minore o maggiore   |
               |          | Esempio: if x < 3 { ... }                    |
               +----------+----------------------------------------------+
               | >= , <=  | Come sopra, includendo l'uguaglianza         |
               |          | Esempio: if x >= 3 { ... }                   |
               +----------+----------------------------------------------+
   ...
   Divieto 
      Non è ammesso in rust l'assegnazione dei valori all'interno di condizioni come
      ad esempio if (x=3) - NON AMMESSO al fine di evitare errori comuni come l'uso  
      accidentale di  = al posto di ==.

   Metodi DI ISTANZA
      metodi blocchi di codice raggruppati che eseguono una specifica azione o calcolo. 
      Definiti in un impl block = blocco di implementazione per un tipo specifico.

         creiamo un blocco di implementazione specifico:
            1) creiamo la struttura rettangolo base x altezza
            ...
            2) definiamo un METODO DI ISTANZA con impl e calcolare l'area
               all'interno di impl viene creato il metodo area

               impl Rettangolo { 
                   
                   // fn = creo la funzione per il Metodo che calcola l'area del rettangolo 
                   fn area(&self) -> u32 {  // ---> u32 restituice il risultato 
                       self.larghezza * self.altezza 
                   } 
               } 

                     Il metodo usa &self → questo significa che opera su un'istanza specifica della struct Rettangolo.
                     self rappresenta il valore dell’oggetto chiamante (mio_rettangolo nel tuo main).


            fn main() { 
                let mio_rettangolo = Rettangolo { 
                    larghezza: 30, 
                    altezza: 50, 
                }; 
            ...
            fn    = dichiaro una funzione all'interno di un blocco impl
            self  = istanza per chiamare il metodo.
                  se lo dichiari self        = viene preso per valore
                  se lo dichiari &self       = viene preso per riferimento
                  se lo dichiari &mut self   = puo essere modificato

            fn area = è associato alla struttura Rettangolo e utilizza i valori 
                     per riferimento &self immutabile ed accede ai campi larghezza, altezza.  


 
   Metodo new = METODI ASSOCIATI 
      Questo metodo associato non richiede  un'istanza dell'oggetto e viene chiamato direttamente sul tipo: 
       il metodo new è il costruttore che crea e restituisce una nuova istanza di Rettangolo.  
         ...
         struct Rettangolo { 
               ...
               E' un metodo statico perche nel blocco impl,  
               Non c'è &self né self: ➜ non serve un'istanza per chiamarlo.
               Lo chiami così: Rettangolo::new(40, 60) ➜ sul tipo, non su un oggetto esistente.

               📦 Terminologia in Rust:

                  ✅ Metodo associato (associated function) = qualunque funzione dentro impl senza self
                  
                  ✅ Metodo di istanza (instance method) = funzione che usa &self, &mut self, o self
                   
               esempio con metodo statico associato e metodo di istanza:
                  ...
                  //esempio 1) METODO ASSOCIATO
                  //Il metodo associato lavora sul tipo (es. Rettangolo) e non su un oggetto.
                  //Viene comunemente usato per creare un nuovo oggetto, come fa il metodo new.
                  impl Rettangolo {
                      // Metodo statico (associato) per tipo
                      fn new(l: u32, h: u32) -> Rettangolo {
                          Rettangolo { larghezza: l, altezza: h }
                      }

                  //esempio 2) METODO DI ISTANZA LAVORA SU UN OGGETTO ESISTENTE preso con self, &self &mut self
                  //Un metodo di istanza viene chiamato su un oggetto esistente, perché utilizza self per accedere ai suoi dati interni.
                      // Metodo di istanza con &self = LAVORA SU UNA ISTANZA ESISTENTE DELLA STRUCT
                      fn area(&self) -> u32 {
                          self.larghezza * self.altezza
                      }
                  }
                  ...
                  fn main() {
                      let r = Rettangolo::new(10, 20); // metodo statico
                      println!("Area: {}", r.area());  // metodo di istanza
                  }


                  ALTRO ESEMPIO DI METODO ASSOCIATO CREA UN NUOVO OGGETTO DA ZERO CON NEW
                           struct Rettangolo {
                                  larghezza: u32,
                                  altezza: u32,
                              }
                              ...
                              impl Rettangolo {
                                  // Metodo associato: lavora sul tipo e crea un oggetto
                                  fn new(l: u32, h: u32) -> Rettangolo {
                                      Rettangolo { larghezza: l, altezza: h }
                                  }
                              }
  
   Metodi Astratti e di Estensione
      Rust non ha metodi astratti, come ad esempio C# o Java dove una CLASSE puo definire  
         dei metodi astratti SENZA IMPLEMENTAZIONE e costringere le classi derivate C# e Java 
         a fornire la implementazione.

         1. ✅ Trait “Astratti” = hanno solo la firma
            sono trait quelli senza metodo di fatto astratti che poi vengono implementati.
            (attenzione che RUST NON HA LE CLASSI quindi non ci sono metodi astratti.)

            Rust non ha classi astratte, ma i trait svolgono un ruolo simile.
            Puoi dichiarare solo la firma dei metodi in un trait, e poi lasciarne l’implementazione ai tipi concreti.


               //trati di fatto astratto perche ha solo la firma   
               // In questo senso, i trait definiscono interfacce astratte.
               trait Forma {
                   fn area(&self) -> f64; // metodo "astratto", senza corpo
               }

         2. ✅ Trait “Asincroni”
         Rust non supporta nativamente i trait asincroni (cioè async fn dentro un trait) 
         perché è difficile da implementare a livello di compilatore.

            Tuttavia, puoi usare workaround:

               a) Con async_trait crate:
                  use async_trait::async_trait;
                     // Funziona grazie al macro system, ma ha un costo in prestazioni e complessità.                     
                     #[async_trait]
                     trait Operazione {
                         async fn esegui(&self);
                     }

         3. ✅ Trait “Generici”
            Assolutamente sì: i trait possono essere generici su tipi.

                  trait Stampa<T> {
                         fn stampa(&self, valore: T);
                     }
                  ...
                  impl<T: AsRef<[i32]>> Somma for T {
                      fn somma(&self) -> i32 {
                          self.as_ref().iter().sum()
                      }
                  }
               
               +-----------+----------------------------+-------------------------------------------------------------+
               | Nome      | Supportato?                | Note                                                        |
               +-----------+----------------------------+-------------------------------------------------------------+
               | Astratto  | ✅                         | I trait in Rust sono simili a interfacce astratte           |
               | Asincrono | ⚠️ Con macro (async_trait) | Non è supportato nativamente, ma possibile con workaround   |
               | Generico  | ✅                         | Pienamente supportato nei trait e nelle loro implementazioni|
               +-----------+----------------------------+-------------------------------------------------------------+

      I trait  sono  simili  alle  interfacce  e  definiscono funzionalità che i tipi possono implementare.
         Un metodo definito in un trait senza un  corpo  è,  DI  FATTO,  UN  METODO  ASTRATTO  che  deve  
         essere  implementato  da  qualsiasi tipo che voglia "implementare" quel trait  

         //trait solo FIRMA 
         trait Forma { 
                fn area(&self) -> f64; // Metodo astratto 
            } 
            //TRAIT implementato dalla struttura cerchio 
            struct Cerchio { 
                raggio: f64, 
            } 
      ...
      Esempio di trait al quale aggiungere nuovi metodi COME IN C#: 

         1) trait solo firma - astratto -
            trait Estensione { 
                   fn descrizione(&self) -> String; 
               } 
         2) implemento la estensione i32 
            //estendo il tipo i32 con il nuovo metodo descrizione 
            impl Estensione for i32 { 
                 //con il metodo descrizione che viene aggiunto 
                fn descrizione(&self) -> String { 
   ...
   METODI ANSINCRONI
      con la parola async rust puo creare funzioni o metodi asincroni.
      Il metodo asincrono restituisce un Future che rappresenta un valore che sara disponibile nel futuro.   
      Questi metodi sono utili per gestire operazioni che richiedono tempo come I/O senza bloccare l'esecuzione
      del programma.
      Il programma simula un attesa di 2 secondi in asincrono.
         esempio: Vedi progetto ---> @async@tokio    
            ...
            impl Downloader { 
                async fn download(&self, url: &str) { 
                    println!("Inizio download da {}", url); 
                    sleep(Duration::from_secs(2)).await; // Simula un'attesa di 2 secondi 
                    println!("Download completato da {}", url); 
                } 
            } 
            ...
            async fn main() { 
                ...
                downloader.download("https://example.com").await; 
            } 
             
         
         In questo esempio, il metodo download è asincrono, permettendo al programma di 
         continuare a funzionare mentre aspetta il completamento dell'operazione simulata. 


   METODI GENERICI
      servono per costuire codice riutilizzabile che puo operare su piu tipi di dati.
         es.
         //contenitore è una struttura gerica <T>
            struct Contenitore<T> { 
                valore: T, 
            } 
             
            //struttura generica che puo contenere qualsiasi valore molto flessibile. 
